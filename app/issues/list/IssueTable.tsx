...
const IssueTable = async ({ searchParams, issues }: Props) => {
                ...
                <Table.Row>
                    {columns.map(column => (
                        <TableColumnHeaderCell key={column.id} className={column.className}>
                            ...
                            }}>{column.label !== 'Created by' && <ArrowUpIcon className='inline nav-link' />}
                            </NextLink>
                            <NextLink href={{
                                query: { ...searchParams, orderBy: column.value, sort: 'desc' }
                            }}>{column.label!=='Created by' &&<ArrowDownIcon className='inline nav-link' />}
                            </NextLink>
 ...
 const columns: {id: number, label: string; value?: keyof Issue; className?: string }[] = [
        ...
        { id: 4, label: 'Created by', className: 'hidden md:table-cell' }
    ];
